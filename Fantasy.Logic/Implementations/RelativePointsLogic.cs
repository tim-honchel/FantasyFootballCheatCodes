...
namespace Fantasy.Logic.Implementations
{
    public class RelativePointsLogic : IRelativePointsLogic
    {
        ...
        {
            if (averages.AverageByPosition.ContainsKey($"{player.Position}1") && averages.AverageByPosition[$"{player.Position}1"] > 0)
            {
                player.RelativePointsByPosition[$"{player.Position}1"] = Math.Round(player.WeeklyPoints - averages.AverageByPosition[$"{player.Position}1"],2);
            }
            if (averages.AverageByPosition.ContainsKey($"{player.Position}2") && averages.AverageByPosition[$"{player.Position}2"] > 0)
            {
                player.RelativePointsByPosition[$"{player.Position}2"] = Math.Round(player.WeeklyPoints - averages.AverageByPosition[$"{player.Position}2"], 2);
            }
            if (averages.AverageByPosition.ContainsKey($"{player.Position}3") && averages.AverageByPosition[$"{player.Position}3"] > 0)
            {
                player.RelativePointsByPosition[$"{player.Position}3"] = Math.Round(player.WeeklyPoints - averages.AverageByPosition[$"{player.Position}3"], 2);
            }
            if (averages.AverageByPosition.ContainsKey($"{player.Position}FA") && averages.AverageByPosition[$"{player.Position}FA"] > 0)
            {
                player.FA = Math.Round(player.WeeklyPoints - averages.AverageByPosition[$"{player.Position}FA"],2);
            }
        }
